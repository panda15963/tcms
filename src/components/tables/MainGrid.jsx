...
const MainGrid = ({ list, onSelectionChange, onCellDoubleClick }) => {
  ...
  // list가 배열인지 확인하고, 아니면 빈 배열로 초기화
  const validList = Array.isArray(list?.list) ? list.list : [];

  // list 변경 시 초기 데이터를 설정
  useEffect(() => {
    if (validList.length > 0) {
      setDisplayedData(validList.slice(0, ITEMS_PER_PAGE)); // 초기 데이터 설정
      setPage(1); // 페이지 초기화
    } else {
      setDisplayedData([]);
    }
  ...
  useEffect(() => {
    ...
    if (JSON.stringify(currentSelectedRows) !== JSON.stringify(selectedRows)) {
      ...
      onSelectionChange(currentSelectedRows); // 선택된 행을 부모 컴포넌트로 전달
    }
  ...
  const handleCellDoubleClick = (rowData) => {
    ...
    if (onCellDoubleClick) {
      onCellDoubleClick(rowData); // 셀 더블클릭 시 부모 컴포넌트로 데이터 전달
    }
  };

  return (
    <div className="my-2 h-[400px] block overflow-x-auto text-center">
      ...
      {displayedData.length < validList.length && (
        ...
        >
          {t('SpaceTable.LoadMore', '더 많은 항목 보기')}
        </button>
      )}
    </div>
  );
