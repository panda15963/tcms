...
export default function PCLists({ selectedPC, setSelectedPC = () => {} }) {
  ...
  // Convert `selectedPC.pcname` into the desired format for the dropdown
  useEffect(() => {
    if (selectedPC && Array.isArray(selectedPC.pcname)) {
      const formattedPCList = [
        { id: 1, name: t('PCList.All') }, // "All" option
        ...selectedPC.pcname.map((name, index) => ({
          id: index + 2, // IDs start from 2
          name,
        })),
      ];
  ...
  const handleOnSelectMap = (selectedMap) => {
    ...
    setSelectedPC(selectedMap); // Sync with parent state
  };

  // Return null if `selected` is not initialized yet
